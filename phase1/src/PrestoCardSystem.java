...
/**
 * Class PrestoCardSystem mainly accept orders from user (now its events.txt) and execute them though other classes.
 * */
class PrestoCardSystem {
    ...
    /** admin users are initiated automatically here since there can only be one boss for this system to check the
     * information.*/
    private AdminUser admin = new AdminUser("Boss", "1234");


    /** method doEvent accept an ArrayList of data read from events.txt (and in phase two, it will accept from user) and
     *  coordinate with other class if necessary*/
    void doEvent(ArrayList<String> data) throws Exception {
        ...
        else if (data.contains("enters") || data.contains("exits")){   // events on enter and exit a station.
            for (User user : users) {
        ...
        else if (data.contains("suspended") || data.contains("retrieved")){   //events on suspend and retrieved card.
            for (User user : users) {
                for (Card card : user.getMyCards()) {
                    if(card.getId() == Integer.parseInt(data.get(1))){
        ...
        else if (data.contains("ended,")){  //events on when the day ended, clear data from the previous day and print report.
            this.admin.report();
