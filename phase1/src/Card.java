...
public class Card {
    ...
    /**Add money to card's balance. Exception will be threw if the adding money is not
     * $10 or $20 or $50. */
    ...
    /**Calculate the amount to deduct from card balance every time the card enters a bus
     * station or exits a subway station. The algorithm also includes the $6 cap in two-hour
     * continuous trip.*/
        ...
        if(vehicle.equals("bus")){
            if (currentTrip.getCurrentFare() + 2 <= 6 & currentTrip.getIsContinuous()) {
                ...
                if (balance >= 0) {
                    ...
                    System.out.println("Card " + id + " new balance: $" + getBalance() +
                            " at " + currentTrip.getEnterTime());
                }
                else{
                    System.out.println("Card " + id + " balance is not enough at " +
    ...
    /**Generate a new trip whenever the card  enters a station. Complete trip information
     * whenever the card exits a station. Call Card.deductFare() at the proper time.*/
