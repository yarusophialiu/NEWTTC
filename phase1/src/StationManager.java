...
/**
 * this class is used to manage all the stations and sort them by their type (subway or bus), and it help used these
 * information to calculate the minimum distance from the source station to the destination station using the simplified
 * version Dijkstra Algorithm where the distance between vertices (in our case, stations) are all 1.
 */
class StationManager {
    ...
    /** this method  will determin whether the station has been created, if its created already, it will return that
     * instance, if its not, it will create a new instance of Station and return it.*/
    ...
    static int minDistance(Station source, Station destination){
        ...
        if (source.getVehicle().equals("subway")){
            // used to set the distance of all stations other than source to infinity since all of them are undiscovered.
            for (Station station :stationSet ) {
                if (station != source){
